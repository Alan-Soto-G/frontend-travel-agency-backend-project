import { Injectable } from '@angular/core';
import { ToastrService } from 'ngx-toastr';
import { Router } from '@angular/router';
import { AuthService } from '../auth.service';

/**
 * Servicio para registro con GitHub OAuth
 *
 * Flujo de registro:
 * 1. Abre popup de GitHub para autorización
 * 2. Usuario autoriza la aplicación
 * 3. GitHub devuelve token de autenticación
 * 4. Se crea/actualiza usuario en el backend
 * 5. Se guarda sesión y se redirige al dashboard
 * @version 1.0
 */
@Injectable({
  providedIn: 'root'
})
export class GithubRegisterService {

  constructor(
    private toastr: ToastrService,
    private router: Router,
    private authService: AuthService
  ) {}

  /**
   * Inicia el flujo de registro con GitHub OAuth
   *
   * @returns Promise con el resultado del registro
   */
  async register(): Promise<boolean> {
    try {
      console.log('🐙 Iniciando registro con GitHub...');

      // Llamar al servicio de autenticación con GitHub
      const result = await this.authService.loginWithGithub();

      if (result) {
        // ✅ Registro exitoso
        console.log('✅ Usuario registrado con GitHub:', result);
        this.toastr.success('¡Bienvenido con GitHub!', 'Registro exitoso');

        // Redirigir al dashboard después de 2 segundos
        setTimeout(() => {
          this.router.navigate(['/main']);
        }, 2000);

        return true;
      } else {
        // ❌ Error en registro
        this.toastr.error('No se pudo completar el registro', 'Error con GitHub');
        return false;
      }
    } catch (error) {
      console.error('❌ Error en registro con GitHub:', error);
      this.toastr.error('No se pudo registrar con GitHub. Intenta de nuevo.', 'Error');
      return false;
    }
  }
}

